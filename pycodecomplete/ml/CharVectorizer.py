...
class CharVectorizer(BaseEstimator, TransformerMixin):
    ...
    def dask_transform(self, raw_documents, copy=True):
        '''Converts string to two boolean encoded vectors X and y. X contains max_length
        number of characters and the target y contains the next character in that sequence.
        ...
        for document in self.documents_to_strings(raw_documents):
            ...
            if X_da is None:
                X_da = da.from_array(X,chunks=(100, 
                                    ...
                                    len(self.tokens)))
                y_da = da.from_array(y,chunks=(self.sequence_length, len(self.tokens)))
            else:
                X_da = da.concatenate([X_da, 
                        da.from_array(X,chunks=(100, 
                                    self.sequence_length,
                                    len(self.tokens)))], axis=0)
                y_da = da.concatenate([y_da,
                        da.from_array(y,chunks=(self.sequence_length, len(self.tokens)))],
                        axis=0)
